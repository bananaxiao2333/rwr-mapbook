...
为什么要切
----------
本站部署在 **EdgeOne Pages**，它**单个文件最大 25 MB**——超了的文件部署不上去。
而地编的整包是 26–84 MB，所以它们只能一切为若干片，随站点一起发布。
...
#: 单片上限：**EdgeOne Pages 的单文件 25 MB 限制**。用十进制，不用 25 MiB ——
#: 托管方说的「25 MB」是哪一种没有明说，按小的那一头切总是安全的（大文件本来就要切）。
#:
#: ⚠️ 这是**部署方**的硬限制，不是 git 的限制：仓库本身能存到 100 MiB 一个文件，
#: 但传上去的时候会被 EdgeOne 挡在门外，部署直接失败。所以「整包存成一个文件、
#: 让下载器直接抓」这条路在本站走不通——超过 25 MB 的整包只能切。
LIMIT = 25 * 1000 * 1000
