...
「下载」多数时候就是一条**直链**：整包在仓库里是一个普通文件，浏览器能下，下载器
也能下。只有超过托管方单文件上限的整包才会被切开，那种由页面把分片取回、逐片核对
哈希，拼成一个**与原文件同名**的整包。两种情况落到下载文件夹里的都是一个整包，
...
!!! note "直链，还是由页面拼"
    **没超过上限的整包就是普通文件**，那个按钮是一条直链：浏览器能下，下载器也能下
    （右键就能交给它），拿到手的就是整包。超过上限的才存成若干片段，那种**只能由
    页面拼**：分片是仓库里的真实地址没错，但那是几段，交给下载器只会拿到几段
    `.partNNN`，还得自己合并。页面把全部片段取回、逐片核对哈希、拼成一个整包再交给
    你；任何一片对不上就停下报错，不会把半个包塞过来。
