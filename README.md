...
| 上限 | 出现在哪 | 后果 |
| --- | --- | --- |
| 25 MB（十进制） | GitHub **网页端**上传 | 超了在浏览器里传不上去 |
| 100 MiB | `git push` | 超了服务端直接拒绝 |

默认按 **100 MiB** 这一档走，也就是 `git push` 那一档：**只在必须的时候才切**。
理由不在上传端，在**下载端**——

* 整包在仓库里是一个普通文件时，页面上那个按钮就是一条**直链**：浏览器能下，
  下载器（IDM、aria2、迅雷）也能下，右键就交给它们了；
* 切成分片则相反。分片本身是真实地址没错，但那是几段，交给下载器只会拿到几段
  `.partNNN`，还得自己合并。拼接这条路用的是 `blob:` 地址——**页面进程内存里的
  临时句柄**，别的程序拿不到，页面一关就没，所以下载器按不动它。

一句话：**分片是给「上传」省事，不是给「下载」省事，代价全落在读者那边。**
只在整包真的超过上限时才付这个代价。

要改回按网页上传那一档切（整包全部 ≤25 MB，代价是下载器用不了）：
`make downloads LIMIT=25000000`。
