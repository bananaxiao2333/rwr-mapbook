...
「下載」多數時候就是一條**直鏈**：整包在倉庫裡是一個普通文件，瀏覽器能下，下載器
也能下。只有超過託管方單文件上限的整包才會被切開，那種由頁面把分片取回、逐片核對
哈希，拼成一個**與原文件同名**的整包。兩種情況落到下載文件夾裡的都是一個整包，
...
!!! note "直鏈，還是由頁面拼"
    **沒超過上限的整包就是普通文件**，那個按鈕是一條直鏈：瀏覽器能下，下載器也能下
    （右鍵就能交給它），拿到手的就是整包。超過上限的才存成若干片段，那種**只能由
    頁面拼**：分片是倉庫裡的真實地址沒錯，但那是幾段，交給下載器只會拿到幾段
    `.partNNN`，還得自己合併。頁面把全部片段取回、逐片核對哈希、拼成一個整包再交給
    你；任何一片對不上就停下報錯，不會把半個包塞過來。
