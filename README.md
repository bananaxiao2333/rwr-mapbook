...
`egg` 那一支是砍版机制的**试用品**，用来说明这套机制是活的：它冻着地编 060
（2026-05-14）那一次构建的真实数字——压缩包与解压后的大小、文件数、
`Assembly-CSharp.dll` 的字节数与 SHA-256，以及 060→0101 逐版的 DLL 大小对照
（470 天的工具增长，用脚本 DLL 的大小当刻度）。它同时是「彩蛋」这么个消遣页，
名字就叫彩蛋，`label` 与目录名都可以随便取。
