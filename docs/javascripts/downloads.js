 ...
 * 两种情况，页面上长得一样，走的路不一样
 * --------------------------------------
 *   整文件（没超过托管方的单文件上限）  按钮换成 `<a download>`，一条**直链**。
 ...
 * 别的程序拿不到，页面一关就没，所以下载器按不动它。要让下载器能用，
 * 地址就必须是服务器上的一个真实文件——这正是「能整份存就整份存」的理由
 * （见 tools/chunker.py 顶部的说明）。
 *
 * 校验
 * ----
...
(function () {
   ...
   * 而拼接那条路用的是 `blob:`，别的程序拿不到。整文件因此不需要本脚本参与下载，
   * 这里只负责把地址摆上去（按钮的文字、类名照搬，样式不变）。
   */
